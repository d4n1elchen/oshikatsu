...
> **Status:** Active roadmap. Phases 1, 2, 2.1, 3 (resolution + hierarchy), 4 (Monitoring & Observability), and 5 (Downstream Export Protocol) have landed. **Phase 6 (Web UI) is in progress** — the read/query layer is in place, the TUI views are migrated to it, the web stack is scaffolded (Hono + Vite + React), and the dashboard's sidebar, hero, streams rail, week strip, event feed, modal, and timeline rail (Feed) all render against real data. Phase 6.1 (Admin Surface) has shipped: `/admin` route with pipeline-health cards, recent-runs rail, review-queue with merge/new actions, canonical-events panel + edit modal, and an extraction-failures panel. Row-level operator-ownership freeze on `normalized_events` keeps operator edits from being clobbered by the resolver. Phase 7 (Multi-Source Support) and Phase 8 (Platform Expansion) follow.
