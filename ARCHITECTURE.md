...
Oshikatsu is a platform for tracking updates about favorite artists and converting them into a unified, analyzable data format. The system ingests data from multiple sources, extracts one event candidate per raw item, resolves those candidates into normalized canonical events (identifying new vs. existing events, merging duplicates, and linking sub-events to their main events), and exports to downstream pipelines.
...
### 4. Event Resolution Engine

Resolves extracted event candidates into canonical normalized events. This is broader than deduplication: the engine performs three intertwined jobs.
...
- **Extracted event**: One event candidate extracted from one raw item. This is source-derived and not yet resolved against existing canonical events.
- **Normalized event**: Canonical event after event resolution. A normalized event may aggregate multiple extracted events and their source provenance, and may carry a `parent_event_id` linking it to a main event when the resolution engine identified it as a sub-event.
...
- **Modularity**: Clean separation between ingestion, extraction, event resolution, storage, and downstream export
...
- **Consistency**: Raw data is extracted into a stable extracted schema, then resolved into canonical normalized events (deduplicated and hierarchically linked)
