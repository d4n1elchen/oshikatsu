...
- **Content filters for the Feed rail.** The Feed rail (raw post timeline) landed in Phase 6 without filters — just the firehose. Add toggles to filter by content type — e.g. hide pure retweets, show only event announcements — once the unfiltered feed is lived in long enough to know which toggles matter. The signals those toggles would consume already exist: `extracted_events.record_kind` distinguishes events from annotations, and `raw_items.status='not_an_event'` separates orphan posts (mood, fan engagement). See `design_docs/2026-05-10-non-event-classification/`.
