...
- **Normal timeline feed with content filters.** A Phase 6 web-UI surface that shows the raw post timeline (not just extracted events), with toggles to filter by content type — e.g. hide pure retweets, show only event announcements. Leans on existing extraction signals to classify posts; toggles let users see the firehose or a curated slice.
